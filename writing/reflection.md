...
Also in regards to my input: 12345678, one will notice that it gives the response that there are "no samples recorded". This does not mean that the function didn't operate correctly it means that the function ran so fast that pyinstrument, which is the profiler that collects the data on the function, can't collect data on the function because it generated the output too quickly. What I mean by this is that pyinstrument collects data every millisecond but if the function finishes running before the millisecond is over then pyinstrument will not be able to collect any data on that function.

There is a simple way to fix this, I can simply edit the `profiler = Profiler()` line of code so that it reads `profiler = Profiler(interval=0.00001)`. This would create an interval in which pyinstrument can collect data that is much less time in between its collection points. However, because 12345678 is not a prime number, I dont think I will be able to generate an interval within the profiler = Profiler() line that will actually generate any data because the number will need to be extremely small becuase the number is not a prime number. Because it is not a prime number, it will run very quickly through the function and I think if I broke down that interval for data collection points any further it would effect the run time for my overall main.py file because it would be stopping to collect data several times. 
